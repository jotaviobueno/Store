...
export class UserRoleDataloader {
  ...
  private async batchUserRole(usersId: string[]): Promise<RoleSchema[][]> {
    const usersRoles = await this.userRoleService.findManyByUsersId(usersId);

    const rolesIds = usersRoles.map((userRole) => userRole.roleId);
